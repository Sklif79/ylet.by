		...
		if(typeof(obPrices) == 'object')
		{
			...
			for(var j in obPrices)
			{
				if(obPrices[j])
				{
					...
					strPrice += '</div>';
					if (obPrices[j].DISCOUNT_VALUE !== obPrices[j].VALUE && this.showOldPrice)
					{
						...
						if(this.showPercent)
						{
							...
							$(this.obPriceOld).closest('.cost').find('.value').html(obPrices[j].DISCOUNT_DIFF_PERCENT_RAW);
							$(this.obPriceOld).closest('.cost').find('.text span').html(obPrices[j].PRINT_DISCOUNT_DIFF);
						}
					}
				}
			}
